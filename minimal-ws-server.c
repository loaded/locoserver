...
static int get_hash_location(struct per_vhost_data__minimal *vhd,struct lws *wsi,struct per_session_data__minimal *pss,json_t *root){
	...
	res = PQexecParams(conn,
		"SELECT title , ST_AsEWKT(geom) , hash   FROM ihash  WHERE ihash.geom && ST_MakeEnvelope($1, $2,$3, $4)",	
			4,
			NULL,
			paramValues,
			NULL,
	...
	for(int i = 0 ; i < rows ; i++){
		...
		json_object_set(child,"title",json_string((const char*)PQgetvalue(res,i,0)));
		json_object_set(child,"x",json_real(OGR_G_GetX(geometry,0)));
		json_object_set(child,"y",json_real(OGR_G_GetY(geometry,0)));		
		json_object_set(child,"hash",json_string((const char*)PQgetvalue(res,i,2)));		
...
static int add_hash_location(struct per_vhost_data__minimal *vhd,struct lws *wsi,
	...
	res = PQexecParams(conn,
			"insert into ihash (geom,title,hash,text,username) values (ST_MakePoint($1,$2),$3,$4,$5,$6)",
			6,
			NULL,
			paramValues,
			NULL,
	...
	json_object_set(response,"route",json_string("profile--upload"));	
	json_object_set(response,"title",json_string(paramValues[2]));
	json_object_set(response,"hash",json_string(paramValues[3]));
...
static int get_hash_hash(struct per_vhost_data__minimal *vhd,struct lws *wsi,struct per_session_data__minimal *pss,json_t *root){
	...
	paramValue[0] = (const char*) bufferX;
	res = PQexecParams(conn,
			"select hash,ST_AsEWKT(geom),title,text,username  from ihash where hash=$1",
			1,
			NULL,
			paramValue,
			NULL,
	...
	json_object_set(response,"route",json_string("profile--hash"));	
...
static int insert_urls(struct per_vhost_data__minimal *vhd,struct lws *wsi,struct per_session_data__minimal * pss){
 ...
 int get_urls(const char* hash,json_t *response){
	...
	paramValue[0] = (const char*) bufferX;
	res = PQexecParams(conn,
			"select url from url where hash=$1",
			1,
			NULL,
			paramValue,
			NULL,
	...
	switch(state){
		...
		case 2 : 
			...
			elapsedTime = (pss->t2.tv_sec -pss->t1.tv_sec);
			   ...
			   printf("lws user and pss->filename \n");

			lwsl_user("%s : file upload done . written %lld \n",__func__,
					pss->filename,pss->total);
			

			printf("lws 3 user and pss->filename \n");

			close(pss->fd);
	
		
			pss->mpload->uploaded++;
			printf("I am433 here and ok\n");

			list_add(pss->mpload->names,filename,strlen(filename)+1);
	
			printf("I am hd adfere and ok\n");
		  ...
		  void *in, size_t len)
{
	...
	char b64[64];
	int n, m;
	const char *resource_path = "/home/namo/geo-hash/ws/www";
	const char *index = "/home/namo/geo-hash/ws/www/index.html";
